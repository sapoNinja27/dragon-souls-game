...
public class Plataforma extends Entity {
	...
	public void tick() {
		...
		for (int i = 0; i < (img.length / 2); i++) {
			img[i] = Game.cenario.getSprite((i) * Configuracoes.TILE_SIZE, 3 * Configuracoes.TILE_SIZE,
			...
			if (atual instanceof Plataforma) {
				if (Entity.isColidding(Game.player, atual, 0, 1)) {
					...
					if (Game.player.clicouBueiros) {
						Game.player.clicouBueiros = false;
						teleportar(Game.escadasDeEsgoto.get(i).getX(), Game.escadasDeEsgoto.get(i).getY(),
								Game.escadasDeEsgoto.get(i).getX(), Game.escadasDeEsgoto.get(i).getY(),
								Game.player.dir);
